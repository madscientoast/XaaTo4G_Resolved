...
f = ROOT.TFile("../Model/shapes_X3000.root", "READ") # I exported the data and original MC to this file. 
data = f.Get("data_obs")
signal = f.Get("signal_m750")

# Create a RooFit workspace and variables
w = ROOT.RooWorkspace("w")

# Define observable (x-axis of the histogram)
mass = ROOT.RooRealVar("x", "x", 180, 3500)
mass.setMin(180)
...
mass.setRange("fullRange", 180, 3500)
...
mass.setRange(180, 3500)

# Define parameters for the double-sided Crystal Ball function (signal)
mean = ROOT.RooRealVar("mean", "mean", 3000, 2800, 3200)
sigma = ROOT.RooRealVar("sigma", "sigma", 100, 50, 200)
alpha1 = ROOT.RooRealVar("alpha1", "alpha1", 1.5, 0.1, 10)
n1     = ROOT.RooRealVar("n1", "n1", 2.0, 0.1, 30)
alpha2 = ROOT.RooRealVar("alpha2", "alpha2", 1.5, 0.1, 10)
n2     = ROOT.RooRealVar("n2", "n2", 2.0, 0.1, 30)
...
# Save workspace
w.Print()
w.writeToFile("workspace_m750.root")